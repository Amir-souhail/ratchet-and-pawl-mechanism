...
The repository includes SolidWorks part and assembly files, detailed technical drawings (with a **Bill of Materials (BOM)**), and rendered images of the final assembly. Additionally, the exploded view of the assembly includes **explode line sketches**, which clearly indicate the connections between different parts of the mechanism.  
...
### **Assembly File**  
- `RatchetAndPawl.SLDASM` — Main assembly file for the mechanism.  

### **Part Files**  
1. `RatchetWheel.SLDPRT` — The toothed wheel that allows motion in one direction.  
2. `Pawl.SLDPRT` — The component that engages with the ratchet teeth to restrict reverse motion.  
3. `Shaft.SLDPRT` — Cylindrical rod that supports the rotation of the ratchet wheel.  
4. `Frame.SLDPRT` — Structural sheet metal support for the assembly.  
5. `Spring.SLDPRT` — A spring that keeps the pawl engaged with the ratchet wheel.  
...
- PDF drawings for all parts and assembly are available in the `Drawings` folder:  
  ...
  - `FrameDrawing.PDF`  
  - `SpringDrawing.PDF`  
  - `RatchetAndPawlAssemblyDrawing.PDF` — Includes a **Bill of Materials (BOM)** and an **exploded view with line sketches** to show the relationships and connections between the assembly components.  

---

## 💻 How to Open the Files  
1. Ensure you have **SolidWorks 2019** or newer installed.  
2. Clone or download this repository to your local machine.  
3. Open the assembly file (`RatchetAndPawl.SLDASM`) in SolidWorks.  
4. Place all part files in the same directory as the assembly file to ensure proper references.  
5. Refer to the `RatchetAndPawlAssemblyDrawing.PDF` for detailed part specifications, **BOM**, and assembly instructions.  

---

## ⚙️ Assembly Components and Working  
